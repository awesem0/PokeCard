...
exports.generatePoem = functions.https.onCall(async (data, _context) => {   
    ...
    try {
        ...
        const poem = response.data.choices[0].message.content.trim().replace(/```/g, '');
        console.log('Parsed poem:', poem);
        if (!poem) {
            throw new functions.https.HttpsError("internal", "Empty poem response");
        }
        return { poem };
    } catch (error) {
